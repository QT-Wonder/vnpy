...
def cache_jq_api(folder: str):
    ...
    folder = Path.cwd().joinpath(folder)
    if not folder.exists():
        folder.mkdir()

    with open(folder.joinpath(underslying_symbol + "_contract_enddate.csv"), mode='w', encoding='utf-8-sig', newline='') as csv_file:
    ...
    with open(folder.joinpath(underslying_symbol + "_future_contract.csv"), mode='w', encoding='utf-8-sig', newline='') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=',')
